...
Use this guide to create a Slack app, collect the Slack keys, configure slash commands, and connect internal Slack threads back to Discord support tickets.
...
Slack uses the Signing Secret to verify slash command requests. Do not use the old verification token.
...
Slack needs a public HTTPS URL for slash commands.
...
The Slack command endpoint in this app is:
...
## 8. Create Slash Command `/self-answer`

This command lets core devs post a final answer from Slack back to the Discord ticket thread.

1. In the Slack app, open **Slash Commands**.
2. Click **Create New Command**.
3. Fill:
   - **Command**: `/self-answer`
   - **Request URL**: `https://your-ngrok-url/slack/commands`
   - **Short Description**: `Post an answer to a Discord support ticket`
   - **Usage Hint**: `SELF-1234ABCD Your answer text`
4. Save.
...
```text
/self-answer SELF-1234ABCD The issue is caused by...
```

The app finds the ticket by ID and posts the answer into the linked Discord thread.

Slack slash commands are normally run from the channel message composer, not inside a thread reply composer. If Slack does not trigger the command from a thread, run it in the main channel with the ticket ID.

## 9. Update The Slack Command After Ngrok Changes

If you already created `/self-answer` and ngrok gives you a new URL:

1. Open https://api.slack.com/apps
2. Select your `Self Support Bot` app.
3. Open **Slash Commands**.
4. Click the pencil/edit icon next to `/self-answer`.
5. Replace **Request URL** with:

```text
https://NEW_NGROK_URL/slack/commands
```

6. Click **Save**.

The command must point to this endpoint:
...
```text
/self-answer SELF-1234ABCD Final answer from the team...
```
